...
class SpecieAgent(Agent):
    ...
    def walk_search_food(self):
        possible_walk_pos = self.model.grid.get_neighborhood(self.pos, moore = self.moore, include_center=True,radius = self.radius)
        # print("Steps possiveis do ponto na pos ", self.pos)
        # print(possible_walk_pos)
    ...
    def get_new_position(self, possible_walk_pos, current_pos):
        ...
        for walk_pos in possible_walk_pos:
            ...
            if len(food) > 0:
                current_distance = (current_pos[0] - walk_pos[0])**2 + (current_pos[1] - walk_pos[1])**2
        ...
        if current_best_pos != None:
            ...
            next_pos = (current_pos[0] + min(self.walk_radius, abs(current_pos[0] - current_best_pos[0]))*step_x
                        ,current_pos[1] + min(self.walk_radius, abs(current_pos[1] - current_best_pos[1]))*step_y)
            
            return next_pos

        next_pos = self.model.grid.get_neighborhood(self.pos, moore = self.moore, include_center=False,radius = self.walk_radius)
